...
def main():
    ...
    print(
        f"Rules: Volume >= {MIN_ATM_VOLUME} | OI >= {MIN_ATM_OI} | Slippage <= {MAX_SLIPPAGE * 100}%"
    )
    print("-" * 100)
    print(
        f"{'SYMBOL':<8} {'PRICE':<8} {'EXP':<12} {'VOL':<8} {'OI':<8} {'SLIP%':<6} {'VOL_RES':<8} {'OI_RES':<8} {'CHANGE'}"
    )
    print("-" * 100)
