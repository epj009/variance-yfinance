...
print("🔬 VARIANCE RESEARCH: Mechanics Impact Study")
print("   Target Strategy: Jade Lizard (30 Delta Put / 30 Delta Call Spread)")
print("   Hypothesis: Low-priced stocks will fail the 'Credit > Width' rule.")
print("-" * 80)
print(
    f"{'SYMBOL':<8} {'PRICE':<8} {'IV':<6} {'PUT($)':<8} {'CALL($)':<8} {'WIDTH':<6} {'CREDIT':<8} {'SCORE':<6} {'STATUS'}"
)
print("-" * 80)
...
for symbol in SYMBOLS:
    try:
        ...
        print(
            f"{symbol:<8} ${price:<7.2f} {iv * 100:.0f}%   {short_put['strike']:<8} {short_call['strike']}/{long_call['strike']:<8} ${real_width:<5.2f} ${total_credit:<7.2f} {score:<6.2f} {status}"
        )
