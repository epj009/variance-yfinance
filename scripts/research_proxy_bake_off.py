...
def run_bake_off():
    ...
    for sym, metrics in data.items():
        ...
        hv = metrics.get("hv252", 0)
        iv_raw = metrics.get("iv", 0)  # This is already normalized in production,
        ...
        if warning == "iv_scale_corrected":
            ...
            vrp_raw = real_raw / hv if hv > 0 else 0
            vrp_fixed = iv_raw / hv if hv > 0 else 0
