...
def _cluster_same_open_date(
    ...
    def _take_named_clusters(size: int) -> None:
        for combo in combinations(leg_infos, size):
            ...
            if any(idx in used_indices for idx in idxs):
                continue
            combo_legs = [c["leg"] for c in combo]
            strat_name = identify_strategy(combo_legs)
            if strat_name in excluded_names:
                continue
            clusters.append(combo_legs)
    ...
    remaining_short_calls = [info for info in short_calls if info["idx"] not in used_indices]
    remaining_short_puts = [info for info in short_puts if info["idx"] not in used_indices]
    used_puts = set()
