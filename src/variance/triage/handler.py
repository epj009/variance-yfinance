"""
Abstract Triage Handler

Defines the contract for individual triage rules in the Chain of Responsibility.
"""

from abc import ABC, abstractmethod
from typing import Any, Generic, Optional, TypeVar

from variance.triage.request import TriageRequest

TriageRequestT = TypeVar("TriageRequestT", bound=TriageRequest)


class TriageHandler(ABC, Generic[TriageRequestT]):
    """Abstract handler in the Chain of Responsibility."""

    def __init__(self, rules: dict[str, Any]):
        self.rules = rules
        self._next: Optional[TriageHandler[TriageRequestT]] = None

    def set_next(self, handler: "TriageHandler[TriageRequestT]") -> "TriageHandler[TriageRequestT]":
        """Sets the next handler in the chain. Returns the next handler for chaining."""
        self._next = handler
        return handler

    @abstractmethod
    def handle(self, request: TriageRequestT) -> TriageRequestT:
        """
        Process the request and add tags if conditions match.

        IMPORTANT: This is a COLLECTOR pattern - always pass to next handler.
        Do NOT short-circuit.
        """
        raise NotImplementedError

    def _pass_to_next(self, request: TriageRequestT) -> TriageRequestT:
        """ALWAYS pass request to next handler (collector pattern)."""
        if self._next:
            return self._next.handle(request)
        return request
