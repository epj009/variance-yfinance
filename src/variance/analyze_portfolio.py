...
def analyze_portfolio(
    ...
    *,
    ...
    portfolio = Portfolio(
        clusters=domain_clusters, net_liquidity=rules.get("net_liquidity", 50000.0), rules=rules
    )
    ...
    if total_portfolio_theta_vrp_adj != 0:
        report["portfolio_summary"]["delta_theta_ratio"] = (
            total_beta_delta / total_portfolio_theta_vrp_adj
        )
    else:
    ...
    for r in all_position_reports:
        if "stale" in r["logic"].lower():
            report["caution_items"].append(
                f"{r['root']}: price stale/absent; tested status uncertain"
            )
    ...
    report["portfolio_summary"]["tail_risk_pct"] = (
        (total_tail_risk / net_liq) if net_liq > 0 else 0.0
    )
