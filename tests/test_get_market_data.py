...
from variance.market_data.providers import TastytradeProvider, YFinanceProvider
from variance.market_data.utils import is_etf, map_symbol, should_skip_earnings
from variance.tastytrade_client import TastytradeAuthError
...
class TestNormalizeIV:
    ...
    def test_normalize_iv_decimal_to_percent_no_context(self):
        ...
        provider = self._provider()
        iv, warning = provider._normalize_iv(0.325, hv_context=None)
        assert iv == 32.5
    ...
    def test_normalize_iv_uses_distance_to_context(self):
        ...
        """
        provider = self._provider()
        iv, warning = provider._normalize_iv(0.25, hv_context=20.0)
        assert iv == 25.0
    ...
    def test_normalize_iv_handles_ambiguous_correctly(self):
        ...
        """
        provider = self._provider()
        iv, warning = provider._normalize_iv(0.5, hv_context=5.0)
        assert iv == 0.5
        assert warning is None
    ...
    def test_normalize_iv_corrects_high_decimal(self):
        ...
        provider = self._provider()
        iv, warning = provider._normalize_iv(1.0, hv_context=15.0)
        assert iv == 100.0
        assert warning == "iv_scale_corrected"
    ...
    def test_normalize_iv_handles_low_vol_context(self):
        ...
        """
        provider = self._provider()
        iv, warning = provider._normalize_iv(1.5, hv_context=5.0)
        assert iv == 1.5
        assert warning is None

    def test_normalize_iv_zero_or_none(self):
        provider = self._provider()
        iv, warning = provider._normalize_iv(0.0, hv_context=10.0)
        assert iv == 0.0
...
class TestCacheHelpers:
    def test_get_cached_market_data_prefers_market_data_key(self, temp_cache_db):
        ...
        result = md_helpers._get_cached_market_data(cache, "AAPL", allow_expired=False)
    ...
    def test_fallback_to_cached_market_data_sets_warning(self, temp_cache_db, monkeypatch):
        ...
        result = md_helpers._fallback_to_cached_market_data(
            cache,
            "AAPL",
    ...
    def test_get_cached_market_data_allows_expired(self, temp_cache_db):
        ...
        result = md_helpers._get_cached_market_data(cache, "AAPL", allow_expired=True)
...
class TestGetPrice:
    def test_get_price_uses_cache(self, temp_cache_db):
        ...
        provider = YFinanceProvider(cache_instance=cache)
        ticker = Mock()

        result = provider._get_price(ticker, "AAPL")
...
class TestCalculateHV:
    def test_calculate_hv_insufficient_history(self, temp_cache_db, monkeypatch):
        ...
        monkeypatch.setattr(md_settings, "HV_MIN_HISTORY_DAYS", 200)
        result = provider._calculate_hv(ticker, "AAPL")
...
class TestGetCurrentIV:
    ...
    def test_get_iv_happy_path(self, temp_cache_db, mock_ticker_factory, mock_option_chain):
        ...
        result = provider._get_current_iv(ticker, 150.0, "AAPL", hv_context=25.0)
    ...
    def test_get_iv_rejects_illiquid(self, temp_cache_db, mock_ticker_factory):
        ...
        provider = YFinanceProvider()
        result = provider._get_current_iv(ticker, 150.0, "AAPL")
        assert result == {}
    ...
    def test_get_iv_uses_nearest_expiration_when_no_dte_match(self, temp_cache_db):
        ...
        result = provider._get_current_iv(ticker, 100.0, "AAPL")
    ...
    def test_get_iv_handles_exception(self, temp_cache_db):
        ...
        result = provider._get_current_iv(BadTicker(), 100.0, "AAPL")
...
class TestProcessSingleSymbolFallback:
    def test_returns_cached_data_on_yfinance_failure(self, temp_cache_db, monkeypatch):
        ...
        provider = YFinanceProvider(cache_instance=cache, market_open_fn=lambda: True)
        monkeypatch.setattr(provider, "_get_price", lambda *_args, **_kwargs: None)

        symbol, data = provider._process_single_symbol("AAPL")
    ...
    def test_market_closed_uses_stale_cache(self, temp_cache_db, monkeypatch):
        ...
        provider = YFinanceProvider(cache_instance=cache, market_open_fn=lambda: False)
    ...
    def test_process_single_symbol_futures_path(self, temp_cache_db, monkeypatch):
        ...
        monkeypatch.setattr(provider, "_get_price", lambda *_args, **_kwargs: (100.0, False))
        monkeypatch.setattr(
            provider,
            "_calculate_hv",
        ...
        monkeypatch.setattr(provider, "_get_current_iv", lambda *_args, **_kwargs: {"iv": 20.0})
        monkeypatch.setattr(provider, "_safe_get_sector", lambda *_args, **_kwargs: "Energy")
        monkeypatch.setattr(provider, "_get_earnings_date", lambda *_args, **_kwargs: "2025-01-01")

        symbol, data = provider._process_single_symbol("/CL")
    ...
    def test_process_single_symbol_allows_after_hours_fetch(self, temp_cache_db, monkeypatch):
        ...
        monkeypatch.setattr(provider, "_get_price", lambda *_args, **_kwargs: (120.0, False))
        monkeypatch.setattr(
            provider,
            "_calculate_hv",
            lambda *_args, **_kwargs: {
        ...
        monkeypatch.setattr(provider, "_get_current_iv", lambda *_args, **_kwargs: {"iv": 24.0})
        monkeypatch.setattr(provider, "_safe_get_sector", lambda *_args, **_kwargs: "Tech")
        monkeypatch.setattr(provider, "_get_earnings_date", lambda *_args, **_kwargs: None)

        symbol, data = provider._process_single_symbol("AAPL", allow_after_hours_fetch=True)
    ...
    def test_process_single_symbol_skips_symbol(self, temp_cache_db, monkeypatch):
        cache = MarketCache(str(temp_cache_db))
        provider = YFinanceProvider(cache_instance=cache)
        monkeypatch.setattr(md_settings, "SKIP_SYMBOLS", {"AAPL"})
    ...
    def test_process_single_symbol_invalid_symbol_dict(self, temp_cache_db):
        ...
        symbol, data = provider._process_single_symbol({"symbol": "AAPL"})
...
class TestMarketDataHelpers:
    def test_map_symbol_prefers_direct_and_root_map(self, monkeypatch):
        monkeypatch.setattr(md_settings, "SYMBOL_MAP", {"AAPL": "AAPLX", "/CL": "CL=F"})

        assert map_symbol("AAPL") == "AAPLX"
        assert map_symbol("/CLZ24") == "CL=F"
        assert map_symbol("") is None

    def test_is_etf_handles_invalid_input(self, monkeypatch):
        monkeypatch.setattr(md_settings, "ETF_SYMBOLS", {"SPY"})

        assert is_etf({"bad": "input"}) is False
        assert is_etf("SPY") is True

    def test_should_skip_earnings_defensive(self, monkeypatch):
        monkeypatch.setattr(md_settings, "SKIP_EARNINGS", {"AAPL"})

        assert should_skip_earnings({"bad": "input"}, "AAPL") is True
        assert should_skip_earnings("AAPL", {"bad": "input"}) is True
        assert should_skip_earnings("/CL", "CL=F") is True
        assert should_skip_earnings("AAPL", "AAPL") is True
    ...
    def test_get_earnings_date_uses_calendar_and_cache(self, temp_cache_db):
        ...
        result = provider._get_earnings_date(ticker, "AAPL", "AAPL")

        assert result == "2025-01-02"

        ticker.calendar = pd.DataFrame()
        result_cached = provider._get_earnings_date(ticker, "AAPL", "AAPL")
    ...
    def test_get_earnings_date_skips_when_configured(self, temp_cache_db):
        ...
        result = provider._get_earnings_date(ticker, "/CL", "CL=F")
    ...
    def test_safe_get_sector_respects_overrides_and_skip_api(self, temp_cache_db, monkeypatch):
        cache = MarketCache(str(temp_cache_db))
        provider = YFinanceProvider(cache_instance=cache)
        monkeypatch.setattr(md_settings, "SECTOR_OVERRIDES", {"AAPL": "Custom"})

        ticker = Mock()
        ticker.info = {"sector": "Technology"}

        assert provider._safe_get_sector(ticker, "AAPL", "AAPL") == "Custom"
        assert provider._safe_get_sector(ticker, "MSFT", "MSFT", skip_api=True) == "Unknown"
    ...
    def test_safe_get_sector_fetches_and_handles_errors(self, temp_cache_db):
        ...
        result = provider._safe_get_sector(ticker, "XOM", "XOM")
        ...
        assert provider._safe_get_sector(broken, "BAD", "BAD") == "Unknown"
...
class TestProviderFallbacks:
    ...
    def test_tastytrade_provider_fallbacks_on_auth_error(self):
        ...
        provider.tt_client.get_market_metrics.side_effect = TastytradeAuthError("auth")
    ...
    def test_yfinance_provider_marks_stale_when_market_closed(self, monkeypatch):
        ...
        provider = YFinanceProvider(market_open_fn=lambda: False)
        monkeypatch.setattr(provider, "_process_single_symbol", fake_process)
        results = provider.get_market_data(["AAPL"])
