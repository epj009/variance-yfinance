...
def mock_rules():
    ...
    return {
        ...
        "stress_scenarios": [
            {"label": "Test Scenario (-1SD)", "sigma": -1.0, "vol_point_move": 5.0}
        ],
...
class TestStressBoxAccuracy:
    def test_per_position_stress_box_calculation(
        self, mock_rules, mock_market_data, mock_portfolio_reports
    ):
        """
...
class TestSizeThreatAccuracy:
    def test_size_threat_for_non_spy_underlying(
        self, msft_cluster_legs, mock_triage_cluster_context
    ):
        """
        ...
        expected_loss = (strategy_delta_bw * move_2sd_spy) + (
            0.5 * strategy_gamma_bw * (move_2sd_spy**2)
        )
